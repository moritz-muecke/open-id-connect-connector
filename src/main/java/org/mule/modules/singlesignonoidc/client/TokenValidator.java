...
public class TokenValidator {
	...
	public TokenValidator(ConnectorConfig config) {
		...
		try {
			publicKey = PemUtils.decodePublicKey(config.getSsoPublicKey());
		} catch (Exception e) {
	...
	public void localTokenValidation(String accessToken) throws VerificationException {
		RSATokenVerifier.verifyToken(accessToken, publicKey, connectorConfig.getSsoUri().toString(), true, true);
	}
}
